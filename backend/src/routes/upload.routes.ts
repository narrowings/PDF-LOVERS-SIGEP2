...
const useR2 = Boolean(process.env['R2_ACCOUNT_ID']) && Boolean(process.env['R2_ACCESS_KEY_ID'])
  && Boolean(process.env['R2_SECRET_ACCESS_KEY']) && Boolean(process.env['R2_BUCKET']);
...
if (useR2) {
  ...
  storage = multerS3({
    s3, bucket: process.env['R2_BUCKET']!, contentType: multerS3.AUTO_CONTENT_TYPE,
    key: (_req: Request, file: Express.Multer.File, cb: (e: Error | null, k: string) => void) => {
      cb(null, `uploads/${crypto.randomBytes(8).toString('hex')}${path.extname(file.originalname).toLowerCase()}`);
    },
  });
} else {
...
const upload = multer({
  ...
  fileFilter: (_req, file, cb) => {
    allowed.includes(path.extname(file.originalname).toLowerCase()) ? cb(null, true) : cb(new Error('Solo PDF, JPG o PNG'));
  },
