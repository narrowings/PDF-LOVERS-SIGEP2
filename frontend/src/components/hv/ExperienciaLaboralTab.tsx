...
interface Item {
  id: string; nombreEntidad: string; cargo: string; tipoInstitucion: string;
  fechaIngreso: string; fechaRetiro?: string; trabajoActual: boolean;
...
export default function ExperienciaLaboralTab({ items, onSaved }: Props) {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccess('Experiencia guardada. Pendiente de validación por el JTH.');
      resetForm(); onSaved();
  ...
  const handleDelete = async () => {
    ...
    try { await hvApi.deleteExperiencia(deleteId); setSuccess('Registro eliminado'); setDeleteId(null); onSaved(); }
    catch (err) { setError(getApiError(err)); }
  ...
  return (
    <div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="table-header">
                {['Entidad','Cargo','Tipo','Ingreso','Estado','Soporte','Validación','Acciones'].map(h =>
            ...
            <tbody>
              {(items as unknown as Item[]).map(item => (
                <tr key={item.id} className="table-row">
                  ...
                  <td className="px-3 py-2">{item.trabajoActual ? <span className="badge-active">Actual</span> : <span className="badge-inactive">Retirado</span>}</td>
      ...
      {showForm && (
        <div className="border border-neutral-200 rounded-lg p-5 bg-neutral-50">
          ...
          <form onSubmit={handleSave} className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div className="sm:col-span-2 lg:col-span-3">
              <FormField label="Certificado laboral — PDF, JPG o PNG · máx. 2 MB">
      ...
      {deleteId && <ConfirmModal title="Eliminar Experiencia" message="¿Está seguro de eliminar este registro?" onConfirm={handleDelete} onCancel={() => setDeleteId(null)} loading={deleting} />}
    </div>
  );
}