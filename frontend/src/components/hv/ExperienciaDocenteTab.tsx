...
export default function ExperienciaDocenteTab({ items, onSaved }: Props) {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccess('Experiencia docente guardada');
      resetForm(); onSaved();
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      await hvApi.deleteDocente(deleteId);
  ...
  const handlePreview = (item: Item) => {
    if (item.documentoUrl) {
      setPreviewTitle(`${item.materiaImpartida ?? item.nivelAcademico} — ${item.institucion}`);
  ...
  return (
    <div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="table-header">
                <th className="px-3 py-2 text-left font-medium">Institución Educativa</th>
                <th className="px-3 py-2 text-left font-medium">Nivel Académico</th>
              ...
              {(items as unknown as Item[]).map(item => (
                <tr key={item.id} className="table-row">
                  ...
                  <td className="px-3 py-2">
                    {item.trabajoActual
                      ? <span className="badge-active">Actual</span>
      ...
      {showForm && (
        <div className="border border-neutral-200 rounded-lg p-5 bg-neutral-50">
          ...
          <form onSubmit={handleSave} className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div className="sm:col-span-2 lg:col-span-3">
              <FormField label="Certificado docente (PDF/JPG · máx. 2 MB)">
                <FileUpload
      ...
      {deleteId && (
        <ConfirmModal
          title="Eliminar Experiencia Docente"
          message="¿Está seguro de eliminar este registro de experiencia docente?"
