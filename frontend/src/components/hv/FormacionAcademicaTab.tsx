...
interface Item {
  id: string; nivelAcademico: string; tituloObtenido: string; institucion: string;
  estadoEstudio: string; fechaGrado?: string; verificadoEdFormal?: boolean;
  verificadoEn?: string; documentoUrl?: string;
}
...
const emptyForm = {
  ...
  institucion: '', programaAcademico: '', tituloObtenido: '', semestresAprobados: '',
  estadoEstudio: 'FINALIZADO', fechaTerminacion: '', fechaGrado: '',
  estudiosExterior: false, documentoUrl: '',
};
...
export default function FormacionAcademicaTab({ items, onSaved }: Props) {
  ...
  const handleEdit = (item: Record<string, unknown>) => {
    ...
    setForm({
      ...
      documentoUrl:      (item['documentoUrl']      as string) ?? '',   // ← punto 6
    });
    setShowForm(true);
  };
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccess('Formación académica guardada. Pendiente de validación por el JTH.');
      resetForm(); onSaved();
  ...
  const handleDelete = async () => {
    ...
    try { await hvApi.deleteFormacion(deleteId); setSuccess('Registro eliminado'); setDeleteId(null); onSaved(); }
    catch (err) { setError(getApiError(err)); }
    ...
    <div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="table-header">
                {['Institución','Título','Nivel','Estado','Soporte','Validación','Acciones'].map(h =>
            ...
            <tbody>
              {(items as unknown as Item[]).map(item => (
                <tr key={item.id} className="table-row">
                  ...
                  <td className="px-3 py-2">
                    {item.documentoUrl
                      ? <span className="text-green-600 text-xs">✓ Adjunto</span>
                      : <span className="text-neutral-300 text-xs">—</span>}
                  </td>
                  <td className="px-3 py-2">
                    {item.verificadoEdFormal
                  ...
                  <td className="px-3 py-2 flex gap-2">
                    {!item.verificadoEdFormal && (
                      <button onClick={() => handleEdit(item as unknown as Record<string, unknown>)}
                    ...
                    {item.verificadoEdFormal && (
                      <span className="text-xs text-neutral-400 italic">Bloqueado</span>
                    )}
                  </td>
                </tr>
              ))}
      ...
      {showForm && (
        <div className="border border-neutral-200 rounded-lg p-5 bg-neutral-50">
          <h4 className="text-sm font-semibold text-primary-700 mb-4">
            {editId ? 'Editar' : 'Agregar'} Formación Académica
          </h4>
          <form onSubmit={handleSave} className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            <FormField label="Nivel Académico" required>
              <select value={form.nivelAcademico}
                onChange={e => setForm(f => ({ ...f, nivelAcademico: e.target.value }))} className="input-field">
                {['BACHILLERATO','TECNICO','TECNOLOGO','PREGRADO','POSTGRADO','MAESTRIA','DOCTORADO','POSTDOCTORADO']
                  .map(n => <option key={n} value={n}>{n}</option>)}
              </select>
            </FormField>
            <FormField label="Institución" required>
              <input type="text" value={form.institucion}
                onChange={e => setForm(f => ({ ...f, institucion: e.target.value }))} className="input-field" required />
            </FormField>
            <FormField label="Título Obtenido" required>
              <input type="text" value={form.tituloObtenido}
                onChange={e => setForm(f => ({ ...f, tituloObtenido: e.target.value }))} className="input-field" required />
            </FormField>
            <FormField label="Programa Académico">
              <input type="text" value={form.programaAcademico}
                onChange={e => setForm(f => ({ ...f, programaAcademico: e.target.value }))} className="input-field" />
            </FormField>
            <FormField label="Área de Conocimiento">
              <input type="text" value={form.areaConocimiento}
                onChange={e => setForm(f => ({ ...f, areaConocimiento: e.target.value }))} className="input-field" />
            </FormField>
            <FormField label="País">
              <input type="text" value={form.pais}
                onChange={e => setForm(f => ({ ...f, pais: e.target.value }))} className="input-field" />
            </FormField>
            <FormField label="Estado del Estudio" required>
              <select value={form.estadoEstudio}
                onChange={e => setForm(f => ({ ...f, estadoEstudio: e.target.value }))} className="input-field">
                <option value="FINALIZADO">Finalizado</option>
                <option value="EN_PROCESO">En Proceso</option>
              </select>
            </FormField>
            <FormField label="Fecha Terminación Materias">
              <input type="date" value={form.fechaTerminacion}
                onChange={e => setForm(f => ({ ...f, fechaTerminacion: e.target.value }))} className="input-field" />
            </FormField>
            <FormField label="Fecha de Grado">
              <input type="date" value={form.fechaGrado}
                onChange={e => setForm(f => ({ ...f, fechaGrado: e.target.value }))} className="input-field" />
            </FormField>
            <FormField label="Semestres Aprobados">
              <input type="number" min={0} max={20} value={form.semestresAprobados}
                onChange={e => setForm(f => ({ ...f, semestresAprobados: e.target.value }))} className="input-field" />
            </FormField>
            <div className="sm:col-span-2 lg:col-span-1 flex items-end">
              <label className="flex items-center gap-2 text-sm text-neutral-600 cursor-pointer">
                <input type="checkbox" checked={form.estudiosExterior}
                  onChange={e => setForm(f => ({ ...f, estudiosExterior: e.target.checked }))} className="rounded" />
                Estudios en el exterior
              </label>
            </div>
            <div className="sm:col-span-2 lg:col-span-3">
              <FormField label="Documento soporte (diploma, acta, certificado) — PDF, JPG o PNG · máx. 2 MB">
                <FileUpload
                  currentUrl={form.documentoUrl || null}
                  onUploaded={url => setForm(f => ({ ...f, documentoUrl: url }))}
                  accept=".pdf,.jpg,.jpeg,.png" maxMB={2} label="Adjuntar documento" />
              </FormField>
            </div>
      ...
      {deleteId && (
        <ConfirmModal title="Eliminar Formación"
